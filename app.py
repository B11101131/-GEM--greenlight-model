...
| 頁面 | 功能 | 說明 |
|------|------|------|
| 🎬 風險分析 | 蒙地卡羅模擬 / VaR / 敏感度 / 情境壓力測試 | 核心模擬引擎與進階風險量化 |
| 💰 財務分析 | 損益兩平 / 現金流 / 投資分攤 | 完整財務模型與投資結構分析 |
| 📊 市場分析 | 歷史票房 / 競爭片 / 檔期建議 | 市場環境分析與上映檔期推薦 |
| 🤖 AI 分析 | 票房預測 / 觀眾輪廓 / 題材熱度 | AI 智能分析與觀眾輪廓預測 |
| 📈 決策總覽 | 決策信號燈 / KPI / PDF・Excel 匯出 | 一頁式決策總覽與報告下載 |
| ⚙️ 系統設定 | 專案管理 / API 金鑰 | 專案歷史紀錄與外部 API 設定 |

### 🚀 使用流程

1. **設定參數** → 在左側邊欄輸入專案基本資訊
2. **執行模擬** → 前往「🎬 風險分析」頁面啟動蒙地卡羅模擬
3. **深入分析** → 查看財務分析、市場分析、AI 分析等頁面
4. **匯出報告** → 在「📈 決策總覽」頁面下載 PDF / Excel 報告
